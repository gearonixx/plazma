...
userver::formats::json::Value Handler::HandleRequestJsonThrow(
    ...
    if (user_row.Empty()) {
        ...
        if (user_row.Empty()) {
            LOG_ERROR() << "POST /v1/auth/login: user row missing after insert, phone=" << dto.phone_number;
    ...
    response["user"]["username"] = user_row.IsNull("username") ? std::string{} : user_row.Get<std::string>("username");
    response["user"]["first_name"] =
        user_row.IsNull("first_name") ? std::string{} : user_row.Get<std::string>("first_name");
    response["user"]["phone_number"] = phone;
    response["user"]["is_premium"] = user_row.IsNull("is_premium") ? false : user_row.Get<bool>("is_premium");
