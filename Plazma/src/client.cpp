        ...
        td_api::downcast_call(
            *update_authorization_state.authorization_state_,
            detail::overloaded(
                ...
                [](td_api::authorizationStateLoggingOut&) { qDebug() << "[AUTH] LoggingOut"; },
                [](td_api::authorizationStateClosing&) { qDebug() << "[AUTH] Closing"; },
                [](td_api::authorizationStateClosed&) { qDebug() << "[AUTH] Closed"; },
                ...
                [](auto& other) {
                    qWarning() << "[AUTH] Unhandled auth state:" << QString::fromStdString(td_api::to_string(other));
                }
            )
        );
    });
...
RequestId TelegramClient::update_request_id() {
    return current_request_id_.fetch_add(1, std::memory_order_relaxed) + 1;
}
